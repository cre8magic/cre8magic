...
public class NamedSettingsCreateCloneSubItems
{
    ...
    private static void MixExpects<T>(NamedSettings<T> h1, NamedSettings<T> h2, NamedSettings<T> expected) where T: DataForTest
    {
        var clone = h1.CloneWith(h2);
        AssertSameAs(expected, clone);
    }

    private static void AssertSameAs<T>(NamedSettings<T> expected, NamedSettings<T> clone) where T: DataForTest
    {
    ...
    private static NamedSettings<T> GetNoCloneEmpty<T>() where T: DataForTest, new() =>
        new()
        {
            { "subitem", new() },
        };

    private static NamedSettings<T> GetNoCloneWithName<T>() where T: DataForTest, new() =>
        new()
        {
            { "subitem", new() { Name = "hello" } },
        };

    private static NamedSettings<T> GetWithId<T>() where T: DataForTest, new() =>
        new()
        {
            { "subitem", new() { Id = 123 } },
        };

    private static NamedSettings<T> GetWithNameAndId<T>() where T: DataForTest, new() =>
        new()
        {
